...
function FleetApp() {
  ...
  const handleUpdateDoc = (
    docId: string,
    update: Pick<MaintenanceDoc, "status" | "notes" | "parts" | "workNotes" | "images" | "timeToFix">,
  ) => {
    setDocs((prev) =>
...
export default function App() {
  ...
  if (session === undefined) {
    return (
      <div className="min-h-full flex items-center justify-center" style={shell}>
        <div style={{ width: 32, height: 32, border: "2px solid var(--color-primary)", borderTopColor: "transparent", borderRadius: "50%", animation: "spin 0.7s linear infinite" }} />
  ...
  if (!session) {
    return <div className="min-h-full" style={shell}><AuthPage /></div>
  }
